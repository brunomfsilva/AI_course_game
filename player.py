...
class Node:
    ...
    def get_children(self, minimizing_player):
        ...
        if minimizing_player is True:
            ...
            legal_pieces, legal_moves = current_state.find_available_moves(WHITE) # I guess same for the player
        ...
        for i in range(len(legal_pieces)):
            for j in range(len(legal_moves[i])):
                ...
                legal_pieces[i].move(legal_moves[i][j][0], legal_moves[i][j][1], state)
...
def minimax(board, depth, alpha, beta, maximizing_player):
    ...
    if depth == 0:
        return evaluate_board(board) # just calculates some evaluation function factor, outputs a number
...
def get_new_state(current_state, piece, new_pos):
    new_state = deepcopy(current_state)
    new_state.chessboard[piece.row][piece.col] = None
    piece.move(new_pos[0], new_pos[1], current_state)
...
def execute_minimax(board, depth, turn):
    ...
    for i in range(len(legal_pieces)):
        for j in range(len(legal_moves[i])):
            new_state = get_new_state(board, legal_pieces[i], legal_moves[i][j])
            new_state_eval = minimax(new_state, depth, -math.inf, math.inf, False)
            if new_state_eval > best_eval:
                best_piece = legal_pieces[i]
                best_move = legal_moves[i][j]
